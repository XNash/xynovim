...
- **Locally patched bacon-ls** (`cmd` override in `lua/plugins/bacon-ls.lua` → `~/.cargo/bin/bacon-ls`, source in `~/.local/src/bacon-ls-0.29.0-patched`): upstream 0.29.0 `abort()`s its debounce task even after the sleep has elapsed — at that point the task *is* the in-flight cargo run, so any keystroke (or auto-save's `didSave`) kills the run silently: the LSP progress token never closes (permanently stacked "checking…" rows) and diagnostics go stale. Three patches, all verified by a 35-check headless E2E suite (`~/.bacon-e2e/`): (1) a generation counter makes runs un-abortable once started — superseded runs are cancelled via `CancelRunning`, which closes tokens properly; (2) with `checkOnSave` off, `didSave` no longer cancels a still-pending live trigger (a save inside the debounce window would silently skip the check); (3) closing a dirty buffer clears its now-orphaned diagnostics and restores the shadow file by *copy* (fresh mtime) instead of hardlink, so cargo re-checks instead of replaying the dirty build's cached warnings. Drop the override when the fixes land upstream.
